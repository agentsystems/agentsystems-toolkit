...
from agentsystems_toolkit.models.router import getModel, _load_model_connection, validate_model_dependencies
...
class TestModelRouter:
    ...
    def test_model_not_configured(self, mock_exists, mock_file, mock_config):
        ...
        with pytest.raises(ValueError, match="No connection configured for model 'missing-model'"):
    ...
    def test_disabled_model(self, mock_exists, mock_file, mock_config):
        ...
        with pytest.raises(ValueError, match="Model connection 'disabled-model' is disabled"):
    ...
    def test_validate_model_dependencies(self, mock_exists, mock_file, mock_config):
        ...
        result = validate_model_dependencies(['claude-sonnet-4', 'gpt-4o', 'missing-model'])
...
class TestLangChainModels:
    ...
    @patch('builtins.open', new_callable=mock_open)
    @patch('os.path.exists')
    def test_anthropic_model_creation(self, mock_exists, mock_file, mock_anthropic, mock_config):
